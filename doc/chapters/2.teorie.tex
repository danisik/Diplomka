...
\newline
\indent Patentový zákon stanovuje, že předmět musí být "užitečný". Výraz "užitečný" v této souvislosti odkazuje na podmínku, že předmět má užitečný účel a zahrnuje také operativnost. To znamená, že stroj, který nebude fungovat\newline k plnění zamýšleného účelu, by nebyl nazván užitečným, a proto by mu nebyl udělen patent. Patent nelze udělit ani na zákony přírody, fyzikální jevy a abstraktní myšlenky.
\newline
...
Ochranná známka je označení pro schopné grafické znázornění, tvořené slovy, písmeny, číslicemi, barvou, kresbou nebo tvarem výrobku či jeho obalu, určené k rozlišení výrobků nebo služeb\cite{ceskoOchranna}, zatímco návrhový patent se vztahuje na okrasný vzhled výrobku jako takového, který není spojen s identifikací zdroje zboží \cite{designTrademark}.
...
\caption{Systém řízení báze dat. Převzato z \cite{img_dbms}.}
\label{fig:dbms}
\end{figure}

\section{Komponenty databáze}
Všechny databáze sestávají z pěti základních komponent, nehledě na použitý typ databáze \cite{TechTargetDB}:
\begin{itemize}
...
\item \textbf{Složitost} - Funkcionality jako například dlouhotrvající transakce, evoluce schémat nebo zpráva verzí činí výsledný systém mnohonásobně složitější, což vede k vyšší ceně a složitějšímu používání.
...
Grafová databáze je typem NoSQL databáze, která je založená na teorii grafů. Data jsou reprezentována jako uzly, hrany zase reprezentují vztahy mezi daty. Graf lze procházet podél určitých typů hran nebo přes celý graf. Procházení spojení nebo relací je velmi rychlé, protože vztahy mezi uzly se nepočítají v době dotazu, ale jsou v databázi trvalé \cite{nosqlDatabaseBook}. Typické příklady jsou: Neo4j, OrientDB, Microsoft Azure CosmosDB. Ukázku grafové databáze lze vidět na obrázku č. \ref{fig:db_img_graph}.
	...
	\caption{Ukázka grafové databáze. Převzato z \cite{neo_graph}.}
...
\noindent Na obrázku č. \ref{fig:cypher_sample} lze vidět jednoduchý dotaz, který hledá výsledný uzel pro vstupní uzel, kterým je člověk se jménem 'Dan', a vztahu 'LOVES' mezi uzly.
	...
	\caption{Dotaz v jazyce Cypher. Převzato z \cite{cypher}.}
