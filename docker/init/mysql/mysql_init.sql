...
----- Views ------
-- 1
CREATE OR REPLACE definer=root VIEW ten_IL_institutes_2015 AS select count(*),
...
-- 2
CREATE OR REPLACE definer=root VIEW five_CA_sections_ge_2010 AS select count(*),
...
-- 3
CREATE OR REPLACE definer=root VIEW twenty_ES_classification_2008 AS select count(*),
count(*) * 100.0 / ((select count(*) from patent_classification
left outer join patents on patents.id = patent_classification.id_patent
...
-- 4
create OR REPLACE definer=root view ten_most_applicants as select count(*),
...
-- 5
create OR REPLACE definer=root view five_languages_2003 as select count(*), count(*) * 100.0 / ((select count(*) from patents
...
-- 6
create OR REPLACE definer=root view ten_ES_authors_most_kinds as select count(distinct classification.section),
count(*) * 100.0 / ((select count(*) from inventors
...
-- 7
create OR REPLACE definer=root view five_most_country_ge_2018 as select count(*),
...
-- 8
create OR REPLACE definer=root view most_FR_kind as select count(*),
count(*) * 100.0 / ((select count(*) from patents
...
-- 9
create OR REPLACE definer=root view fifteen_UK_author_textile_2013 as select count(*),
count(*) * 100.0 / ((select count(*) from inventors
left outer join patents on patents.id = inventors.id_patent
left outer join patent_classification on patents.id = patent_classification.id_patent
left outer join classification on classification.id = patent_classification.id_classification
